...
public class OffScreenIndicator : MonoBehaviour
{
    ...
    void DrawIndicators()
    {
        ...
        {
            Vector3 screenPosition = OffScreenIndicatorCore.GetScreenPosition(mainCamera, target.IndicatorPoint.position);
            ...
            if(indicator)
            {
                indicator.SetAlpha(GameManager.Ins.IsState(GameState.MainMenu) ? 0 : 1);
                indicator.SetName(target.Name);
                indicator.SetScore(target.Score);
                indicator.SetColor(target.TargetColor);// Sets the image color of the indicator.
